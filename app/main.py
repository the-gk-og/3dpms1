...
_REFERENCE_MODELS = {'request': Request, 'quote': Quote, 'job': Job, 'invoice': Invoice}
...
@main_bp.route('/reference/<entity_type>/<int:entity_id>/edit', methods=['POST'])
@login_required
def edit_reference_number(entity_type, entity_id):
    """Edit the reference number on one pipeline stage. By default the new value
    is propagated to every other stage in the same chain (request/quote/job(s)/
    invoice(s)) that is currently blank or already matches. If a linked stage has
    a *different* non-blank reference number \u2014 typically legacy data that predates
    this field, or rows brought in via CSV import \u2014 nothing is overwritten until
    the user resolves the conflict explicitly.
    """
    model = _REFERENCE_MODELS.get(entity_type)
    if not model:
        abort(404)
    record = model.query.get_or_404(entity_id)

    new_value = (request.form.get('reference_number') or '').strip()
    force_all = request.form.get('resolution') == 'overwrite_all'
    overwrite_keys = set(request.form.getlist('overwrite'))

    chain = get_reference_chain(record)

    conflicts = []
    for r in chain:
        if r is record:
            continue
        key = f'{reference_chain_type_key(r)}:{r.id}'
        current = (r.reference_number or '').strip()
        if not current or current == new_value or force_all or key in overwrite_keys:
            continue
        conflicts.append(r)

    if conflicts and not force_all and request.form.get('confirmed') != '1':
        return render_template(
            'reference_conflict.html',
            record=record, entity_type=entity_type, new_value=new_value,
            conflicts=conflicts, type_key=reference_chain_type_key,
            detail_url=lambda r: _REFERENCE_DETAIL_ENDPOINTS[reference_chain_type_key(r)](r),
        )
    ...
    for r in chain:
        key = f'{reference_chain_type_key(r)}:{r.id}'
        current = (r.reference_number or '').strip()
        if r is record or not current or current == new_value or force_all or key in overwrite_keys:
            r.reference_number = new_value
            updated += 1

    log_audit(
        'reference_number_updated', target_type=entity_type, target_id=record.id,
        detail=f'set to "{new_value}" across {updated} linked record(s)',
    )
    db.session.commit()

    skipped = len(chain) - updated
    msg = f'Reference number updated across {updated} record{"s" if updated != 1 else ""}.'
    if skipped:
        msg += f' {skipped} linked record{"s" if skipped != 1 else ""} left unchanged (different reference number).'
    flash(msg)
    return redirect(_REFERENCE_DETAIL_ENDPOINTS[entity_type](record))


@main_bp.route('/reference-audit')
@login_required
def reference_audit():
    """Maintenance page listing every pipeline chain where the reference number
    doesn't line up across its stages \u2014 the situation legacy data or CSV imports
    that predate this field can leave behind."""
    mismatches = find_reference_number_conflicts()
    return render_template('reference_audit.html', mismatches=mismatches, type_key=reference_chain_type_key)
